...
//   ✅ All setSetting() calls auto-sync to MongoDB
...
// ── MongoDB model (lazy — only used after mongoose connects) ──
...
//    Call this once at startup (await loadSettingsFromDB())
...
async function saveSettings(settings) {
    ...
    _cache = settings;
    // Save to file immediately (sync — fast)
    saveToFile(settings);
    // Save to MongoDB async (non-blocking)
    saveToMongo(settings).catch(() => {});
    return true;
}
...
module.exports = {
    loadSettings,
    loadSettingsFromDB,   // ← call this once at startup in index.js
    saveSettings,
    getSetting,
    setSetting,
    setSettings,
