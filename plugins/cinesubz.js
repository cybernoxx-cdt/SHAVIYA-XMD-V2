...
cmd({
  ...
  try {
    ...
    if (downloadLinks && downloadLinks.length) {
      console.log("CINESUBZ available link names:", downloadLinks.map(function(v) { return v.name; }).join(", "));
    }

    var PREFERRED = ["pix", "direct", "gdrive", "mega", "mediafire", "dropbox", "unknown"];
    var selected = null;

    if (downloadLinks && downloadLinks.length) {
      // Try preferred names in order first
      for (var pi = 0; pi < PREFERRED.length; pi++) {
        selected = downloadLinks.find(function(v) { return v.name && v.name.toLowerCase() === PREFERRED[pi]; });
        if (selected && selected.url) break;
      }
      // If none matched, just take the first link that has a URL
      if (!selected || !selected.url) {
        selected = downloadLinks.find(function(v) { return v.url; });
      }
    }

    if (!selected || !selected.url) {
      var debugNames = downloadLinks ? downloadLinks.map(function(v) { return v.name; }).join(", ") : "none";
      return reply("❌ *Download links not available*\n\nAPI returned: [" + debugNames + "]\nPlease try another quality or movie.");
    }
